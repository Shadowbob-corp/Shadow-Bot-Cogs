...
class Core:
    ...
    async def play_allin(self, ctx, bet, multiplier):
        ...
        if outcome == 0:
            msg = _('''▂▃▅▇█▓▒░ <a:whiteplayingcards:1054519560180613240> <a:blackplayingcards:1054517395189944380> <a:whiteplayingcards:1054519560180613240> <a:blackplayingcards:1054517395189944380> ░▒▓█▇▅▃▂''')
    ...
    @game_engine("Coin", (_("heads"), _("tails"),_("h"),_("t")))
    async def play_coin(self, ctx, bet, choice):
        message = await ctx.send(_("The coin flips into the air..."))
        await asyncio.sleep(2)
        outcome = random.choice((_("heads"), _("tails"),_("h"), _("t")))
        if outcome == "heads" or outcome == "h":
            msg = _("The coin landed on {}!").format("heads")
        else:
            msg = _("The coin landed on {}!").format("tails")
...
class Blackjack:
    ...
    async def blackjack_game(self, ctx, amount):
        ...
        if choice.content.lower() == _("double"):
            ...
            ph, dh, msg = await self.bj_loop(ctx, ph, dh, ph_count, condition2, message=msg)
            dh = self.dealer(dh)
            return ph, dh, amount, msg
