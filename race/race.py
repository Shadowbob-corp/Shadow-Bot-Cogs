...
__author__ = "Redjumpman, Geet, Jay_"
__version__ = "2.1.6"
...
class Race(commands.Cog):
    ...
    async def remove(self, ctx):
        for i in ctx.author.roles:
            if i.id== 1045300412305121362:
                mem = ctx.guild.get_member(ctx.author.id)
                mem.remove_roles(ctx.guild.get_role(1045300412305121362),reason="none")
                await ctx.send(f'{ctx.author.mention} can\'t handle the competition, don\'t bother adding them to the race anymore.')
    ...
    async def start(self, ctx):
        ...
        if not has_racer:
            await ctx.send(        
            f'{ctx.author.mention} you don\'t have the racer role, add it now? (say yes)'
        )
        try:
            ...
            if answer == 'yes':
                mem = ctx.guild.get_member(ctx.author.id)                
                await mem.add_roles(ctx.guild.get_role(1045300412305121362), reason=f"Start your bitch-ass engines")
                await ctx.send(f'{ctx.author.mention} is now a super speed racer, feel free to add them to the race with >race add @{ctx.author}!\nUse >race remove to remove the role.')
        ...
        await ctx.send(
            f" 𝙷𝚎𝚢 𝚖𝚘𝚝𝚑𝚎𝚛𝚏𝚞𝚌𝚔𝚎𝚛𝚜 𝚠𝚎'𝚛𝚎 𝚊𝚋𝚘𝚞𝚝 𝚝𝚘 𝚜𝚝𝚊𝚛𝚝 𝚛𝚊𝚌𝚒𝚗𝚐 Type {ctx.prefix}race enter "
            f"𝚝𝚘 𝚓𝚘𝚒𝚗 or {ctx.prefix}race add @user to add someone! (you can only add people with the Racer role)\nThe race will begin in "
            f"{wait} seconds!\n\n**{ctx.author.mention}** entered the race!"
        )

        await asyncio.sleep(wait)
        self.started[ctx.guild.id] = True
        await ctx.send("<a:redsiren:1041917759249977455> 𝐀 𝐅𝐔𝐂𝐊𝐈𝐍𝐆 𝐑𝐀𝐂𝐄 𝐇𝐀𝐒 𝐒𝐓𝐀𝐑𝐓𝐄𝐃! 𝐈𝐌 𝐆𝐎𝐍𝐍𝐀 𝐂𝐔𝐌! <a:redsiren:1041917759249977455>")
    ...
    async def bet(self, ctx, bet: int, user: discord.Member):
        ...
        if len(self.players[ctx.guild.id]) < 3:
            return await ctx.send(
                "Bets are only allowed with 3 or more racers!"
            )
    ...
    async def add(self, ctx, user: discord.Member):
        ...
        if has_racer:
            ...
            elif user in self.players[ctx.guild.id]:
                return await ctx.send("They are already in the race.")
            elif len(self.players[ctx.guild.id]) >= 14:
                return await ctx.send("The maximum number of players has been reached.")
            else:
                self.players[ctx.guild.id].append(user)
                await ctx.send(f"{user.mention} has been added to the race.")
        else:
            await ctx.send(f'{user.mention} doesn\'t have the racer roll, they can only enter themselves.')



    @race.command(alias="ren")
    ...
    async def mode(self, ctx, mode: str):
        ...
        if mode.lower() not in ("zoo", "special",  "normal", "slowboys"):
    ...
    async def on_message(self, message):
        if len(self.waiting) == 0:
            return
        if message.author.id in self.waiting:
            if message.content.lower() == 'yes':
                await message.channel.send(f'{message.author.mention} now has the racer role, force them to race with >race add @{message.author}!')
    ...
    async def _build_end_screen(self, ctx, settings, currency, color):
        ...
        embed = discord.Embed(colour=color, title="<a:trophyblb:1041930705661341696> <a:blackouthehe:1040387875952398447> 𝐂𝐇𝐀𝐌𝐏𝐈𝐎𝐍𝐒 𝐎𝐅 𝐁𝐋𝐀𝐂𝐊𝐎𝐔𝐓𝐒 𝐑𝐀𝐂𝐄 <a:blackouthehe:1040387875952398447> <a:trophyblb:1041930705661341696>")
        embed.add_field(name=f"{first[0].display_name} 1st", value=first[1].emoji)
        embed.add_field(name=f"{second[0].display_name} 2nd", value=second[1].emoji)
        if third:
            embed.add_field(name=f"{third[0].display_name} 3rd", value=third[1].emoji)
        ...
        while not all(animal.position == 0 for animal, jockey in players):
            ...
            for animal, jockey in players:
                if animal.position == 0:
                    fields.append(f"<a:checkmeoffblb:1041934862002692126> **{animal.current}**   [{jockey.display_name}]")
                    continue
                animal.move()
                fields.append(f"<a:checkmeoffblb:1041934862002692126> **{animal.current}**   [{jockey.display_name}]")
