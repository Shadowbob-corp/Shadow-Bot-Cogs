...
class Cashdrop(commands.Cog):
    ...
    async def on_message(self, message):
        if message.guild.id not in self.server_list:
            return
        ...
        if message.guild.id not in self.cache:
            return
        if message.guild.id in message.guild.voice_channels:
            return
        ...
        if self.cache[message.guild.id]["maths"]:
            ...
            if not pred.result:
                ...
                await msg.edit(
                    content=f"Correct! {message.author.mention} got {creds} {await bank.get_currency_name(guild=message.guild)}!"
                )
